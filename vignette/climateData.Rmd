...
# }
#   
#   save(asmrNorth,file=here::here("dat","asmrNorth.rda"))
...
```{r}
# Create a lookup table to assign subzones to their respective variant with climate data
# bgc.miss=bgc unit missing climate data
# bgc.comp=bgc unit with climate station data
look.tab<-
  tibble(bgc.miss=northBGC[!northBGC%in%asmrANNUAL$bgc]) %>% 
  mutate(bgc.comp="ESSFmv2") %>% 
  mutate(bgc.comp=replace(bgc.comp,str_detect(bgc.miss,'SBSmc'),"SBSmc2")) %>% 
  mutate(bgc.comp=replace(bgc.comp,str_detect(bgc.miss,'MHmm2'),"NA")) %>% 
  mutate(bgc.comp=replace(bgc.comp,str_detect(bgc.miss,'ICHwk'),"ICHwk1")) %>% 
  mutate(bgc.comp=replace(bgc.comp,str_detect(bgc.miss,'ICHmc'),"ICHmc2")) %>%
  mutate(bgc.comp=replace(bgc.comp,str_detect(bgc.miss,'SBSwk'),"SBSwk1")) %>% 
  mutate(bgc.comp=replace(bgc.comp,str_detect(bgc.miss,'ESSFxv'),"NA")) %>%
  mutate(bgc.comp=replace(bgc.comp,str_detect(bgc.miss,'ESSFwk'),"NA")) %>% 
  mutate(bgc.comp=replace(bgc.comp,str_detect(bgc.miss,'ESSFmm'),"NA")) %>% 
  mutate(S1.mean=NA,S2.mean=NA,S3.mean=NA,S4.mean=NA,S5.mean=NA)
